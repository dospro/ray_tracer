...
describe('Creating rays', () => {
    ...
    test('Computing a point from a distance', () => {
        const r = ray(CTuple.make_point(2, 3, 4), CTuple.make_vector(1, 0, 0));
        expect(position(r, 0)).toStrictEqual(CTuple.make_point(2, 3, 4));
        expect(position(r, 1)).toStrictEqual(CTuple.make_point(3, 3, 4));
        expect(position(r, -1)).toStrictEqual(CTuple.make_point(1, 3, 4));
        expect(position(r, 2.5)).toStrictEqual(CTuple.make_point(4.5, 3, 4));
    });
});

describe('Transforming rays', () => {
    test('Translating a ray', () => {
        const r = ray(CTuple.make_point(1, 2, 3), CTuple.make_vector(0, 1, 0));
